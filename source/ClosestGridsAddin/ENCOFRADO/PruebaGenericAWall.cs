    ...
    public class ConvertGenericToWallOrFloorCommand : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                // Lista de IDs de encofrados a eliminar DESPUÉS de crear muros/suelos
                List<ElementId> encofradosAEliminar = new List<ElementId>();

                // DETECCIÓN: ¿Es un solo encofrado? Si es así, usar modo MINIMALISTA
                bool modoMinimalista = (encofrados.Count == 1);
                if (modoMinimalista)
                {
                    _log.AppendLine("\n⚠ MODO MINIMALISTA activado (un solo encofrado)");
                    _log.AppendLine("  El muro se creará SIN copiar parámetros ni desplazar para evitar rollback");
                }

                // PRIMERA TRANSACCIÓN: Crear muros/suelos SIN eliminar encofrados
                using (Transaction trans = new Transaction(_doc, "Crear Muros/Suelos desde Encofrados"))
                {
                    try
                    {
                        trans.Start();

                        // DESHABILITADO TEMPORALMENTE: FailurePreprocessor podría estar causando rollback
                        // FailureHandlingOptions failOpt1 = trans.GetFailureHandlingOptions();
                        // failOpt1.SetFailuresPreprocessor(new FailurePreprocessor());
                        // failOpt1.SetForcedModalHandling(false);
                        // failOpt1.SetClearAfterRollback(true);
                        // failOpt1.SetDelayedMiniWarnings(true);
                        // trans.SetFailureHandlingOptions(failOpt1);

                        _log.AppendLine($"⚠ FailurePreprocessor DESHABILITADO para diagnóstico");

                        foreach (Element encofrado in encofrados)
                        {
                            try
                            {
                                int metodoUsado = ProcesarEncofrado(encofrado, ref murosCreados, ref suelosCreados, encofradosAEliminar);
                        ...
                        if (status != TransactionStatus.Committed)
                        {
                            ...
                            // LIMPIAR la lista para NO eliminar encofrados si la transacción falló
                ...
                // SEGUNDA TRANSACCIÓN: Eliminar encofrados originales
